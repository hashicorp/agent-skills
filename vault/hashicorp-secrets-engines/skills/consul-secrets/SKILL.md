...
### "My app needs Consul ACL tokens for service mesh"
→ Create a **service identity role**. [Jump to Service Identity Roles](#service-identity-roles-consul-15)

### "I need read-only Consul KV access for my app"
→ Create a **policy-based role**. [Jump to Policy-Based Roles](#policy-based-roles-consul-14)

### "I want short-lived Consul tokens with automatic rotation"
→ Configure **TTL on roles**, Vault handles revocation. [Jump to Lease Management](#lease-management)

### "I need Consul tokens scoped to specific namespaces (Enterprise)"
→ Use **namespace-scoped roles**. [Jump to Consul Enterprise](#consul-enterprise-features)

---

## How Consul Secrets Engine Works

1. **Configure** → Vault connects to Consul with a management token
2. **Create roles** → Define what ACL policies/identities tokens will have
...
| Your Need | Role Type | Configuration |
|-----------|-----------|---------------|
| KV/service read access | Policy-based | `consul_policies="readonly"` |
| Connect sidecar proxy | Service Identity | `service_identities="web:dc1"` |
| Node-level operations | Node Identity | `node_identities="server-1:dc1"` |
| Namespace-scoped (Enterprise) | Namespace | `consul_namespace="team-a"` |
